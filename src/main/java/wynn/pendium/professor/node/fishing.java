...
public class fishing {
    ...
    public static void register(BlockPos origin, String name, int level, boolean canHarvest) {
        ...
        professor.saveNode("FISH", name, barrier, level);
        nodes.add(new Node(barrier, getCheckMark(barrier, level), barrier.add(0,4,0), (byte) 0, name, level, false, NodeType.FISH));
    }

    public static void load(BlockPos barrier, String name, int level) {
        if (nodeExists(barrier)) return;
        nodes.add(new Node(barrier, new BlockPos(0,0,0), barrier.add(0,4,0), (byte) 0, name, level, false, NodeType.FISH));
    }
    ...
    static BlockPos getCheckMark(BlockPos barrier, int level) {
        if (level == 90 || level == 110) return barrier.add(0, 2, 0);
    ...
    public static void update() {
        for (int i = 0; i < nodes.size(); i++) {
            ...
            if (nodes.get(i).needRecalc())
                nodes.get(i).setCheckMark(getCheckMark(nodes.get(i).getBarrier(), nodes.get(i).getLevel()));
        }
    }
